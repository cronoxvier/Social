...
const reversePayment = async (req: Request, res: Response) => {
  try {
    ...
    await axios.post(url, dataValues).catch((e) => console.log(e));
  } catch (error) {
    //console.log(error)
    res.status(500).send({
      error,
  ...
  try {
    ...
    await axios
      ...
      .then(async (e) => {
        console.log(e, "e");
        respon = e.data;
        result = await placeToPayRequestId.create({
          ...data,
...
const checkOut = async (req: Request, res: Response) => {
  try {
    ...
    const { internalReference, id } = await placeToPayRequestId.findOne({
      where: { order_id },
    });
    ...
    await axios
      ...
      .then(async (e) => {
        ...
        const placeUpdate = await placeToPayRequestId.update(
          { ...datos },
  ...
  try {
    ...
    await axios
      ...
      .then(async (e) => {
        ...
        const placeUpdate = await placeToPayRequestId
          .update(
            { ...datos },
            { where: { requestId: requestId }, returning: true }
          )
          .then((res) => {});
      })
      .catch((err) => {
...
const notify = async (req: Request, res: Response) => {
  try {
    const { ...data } = req.body;
    const place = await placeToPayRequestId.findOne({
    ...
    if (hash != data.signature) {
      ...
      if (place.pharmacy_id != null && place.user_id != null) {
        ...
        let expo = new Expo({
          accessToken: process.env.EXPO_ACCESS_TOKEN, //'DTkss6v_Z9qdGvEZLwI2D0_eTs5h-M4XF6Wx5leW'
        });
        const pushToken = place.token_client;
        let messages = [];
        const PlaceToPayRequest = await placeToPayRequestId.findOne({
          where: { requestId: data.requestId },
        });
        let state;
        ...
        await Order.update(
          ...
          { where: { id: PlaceToPayRequest.order_id } }
        );
      }
      // if (place.pharmacy_id != null && place.user_id == null) {
...
const updateRequestIdStatus = async (req: Request, res: Response) => {
  try {
    ...
    const place = await placeToPayRequestId.update(
      { ...data },
      { where: { requestId: requestId }, returning: true }
    );
  ...
  try {
    ...
    await axios
      ...
      .then(async (e) => {
        ...
        await placeToPayRequestId.update(
          { ...newData },
          {
            where: {
              ads_id: AdsId,
